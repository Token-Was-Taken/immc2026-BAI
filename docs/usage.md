...
所有 pipeline 脚本共享统一的输入 JSON 格式。以下按功能模块分组说明全部配置参数。
...
### 3.5 风险模型权重 (risk_model_config)

详见第四章「风险模型」中的权重参数说明。

### 3.6 物种配置 (species_config)
...
| `use_risk_priority` | bool | false | 启用风险优先部署模式，优先将资源部署到高风险网格 |
| `high_risk_percentage` | float | 0.3 | 高风险网格占比（0.0-1.0），前 X% 的高风险网格被优先部署 |
...
### 3.9 覆盖效果折扣 (coverage_effectiveness)
...
### 4.5 时间因子（可选，`use_temporal_factors: true` 时启用）

时间因子作为乘数叠加在基础风险上，夜间雨季的综合风险最高（×1.56）。默认关闭，适合需要分析特定时段风险的场景。

#### 时间因子计算模式

`DiurnalFactorCalculator` 支持两种昼夜因子计算模式，由 `diurnal_mode` 参数控制：

| 模式 | 使用的参数 | 计算方式 |
| :--- | :------- | :------- |
| **DISCRETE**（默认） | `daytime_factor`, `nighttime_factor` | 白天（6:00-18:00）用 `daytime_factor`，其他时间用 `nighttime_factor`，**gamma 参数被忽略** |
| **CONTINUOUS** | `gamma` | 24 小时正弦波变化，**忽略 `nighttime_factor`** |

**DISCRETE 模式（默认）**：

```python
# 白天（6:00-18:00）
T_t = daytime_factor  # 默认 1.0

# 夜间（18:00-6:00）
T_t = nighttime_factor  # 默认 1.3
```

此模式下，`gamma` 参数虽然被读取和存储，但**不会参与任何计算**。这是当前的默认行为。

**CONTINUOUS 模式**：

```python
# 基于正弦波的连续变化
T_t = 1.0 + gamma * sin(2π * (hour - 6) / 24)
```

此模式下，`nighttime_factor` 参数**不会参与计算**，昼夜因子完全由 `gamma` 控制。

| 小时 | 0 | 3 | 6 | 9 | 12 | 15 | 18 | 21 |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| DISCRETE | 1.3 | 1.3 | 1.0 | 1.0 | 1.0 | 1.0 | 1.3 | 1.3 |
| CONTINUOUS (γ=0.3) | 1.3 | 1.15 | 1.0 | 1.15 | 1.3 | 1.15 | 1.0 | 1.15 |

> **注意**：当前代码中 `diurnal_mode` 参数**未从 JSON 配置中读取**，始终使用默认的 DISCRETE 模式。若需要使用 CONTINUOUS 模式，需修改代码显式传递 `mode=DiurnalMode.CONTINUOUS`。

#### 时间因子配置参数

| 参数 | 默认值 | 说明 |
| :--- | :---: | :--- |
| `daytime_factor` | 1.0 | 白天风险因子（DISCRETE 模式使用） |
| `nighttime_factor` | 1.3 | 夜间风险因子（DISCRETE 模式使用，夜间风险通常更高） |
| `gamma` | 0.3 | 昼夜正弦波振幅（CONTINUOUS 模式使用，值域 [0, 1]） |
| `dry_season_factor` | 1.0 | 旱季风险因子基准值 |
| `rainy_season_factor` | 1.2 | 雨季风险因子，雨季盗猎活动可能更频繁 |
...
- **原始风险**：
  - DISCRETE 模式：夜间 ≈ 1.3 × 白天
  - CONTINUOUS 模式：白天到夜间平滑过渡，夜间最高约 1.3 × 正午
- **归一化风险**：不同时段的 min/max/mean 统计值相同（都是 \[0, 1] 范围）
...
### 5.1 Protection Pipeline（核心流程）

**文件**：`hexdynamic/protection_pipeline.py`

> 推荐使用 `run.py`（见 5.2）一键完成优化+可视化。`protection_pipeline.py` 适合只需要输出 JSON、不需要图片的场景。
...
# 热启动（从已有输出 JSON 加载部署方案作为初始解）
python protection_pipeline.py <input.json> <output.json> --warm-start previous_output.json
```
...
| `--warm-start`               | None            | 热启动：从已有输出 JSON 加载部署方案作为初始解                              |
...
```jsonc
{
  "summary": {
    "total_grids": 120,
    "total_risk": 45.23,
    "total_risk_weighted": 58.80,
    "best_fitness": 0.498,
    "total_protection_benefit": 33.2,
    "average_protection_benefit": 0.277,
    "risk_min": 0.0, "risk_max": 1.0, "risk_mean": 0.45,
    "raw_risk_min": 0.12, "raw_risk_max": 0.89, "raw_risk_mean": 0.48,
    "residual_risk_min": 0.0, "residual_risk_max": 0.85, "residual_risk_mean": 0.32,
    "total_residual_risk": 28.1,
    "fitness_history": [...],
    "resources_deployed": { "total_cameras": 10, "total_drones": 3, ... }
  },
  "visualization_config": { "show_grid_ids": false },
  "grids": [
    {
      "grid_id": 0, "q": 0, "r": 0, "x": 0, "y": 0,
      "terrain_type": "SparseGrass",
      ...
      "raw_risk": 0.42,
      "protection_benefit_raw": 0.22,
      "protection_benefit_normalized": 0.43,
      "residual_risk_normalized": 0.18,
      "deployment": {
...
| `raw_risk`                      | 归一化前的原始风险值                     | 含时间因子的绝对风险值          |
...
| `--prefix`                   | `""`            | 输出文件名前缀                                                  |
| `--grid_dpi`                 | 自动计算            | 每个六边形网格在输出图片中占用的像素宽度（默认根据地图大小自动计算，最大80）                  |
| `--dpi`                      | 150             | matplotlib savefig 的 DPI，控制输出图片的打印分辨率                    |

### 5.3 批量 Pipeline（多场景优化+可视化）
...
| `--grid_dpi`                 | 自动计算              | 每格像素宽度（默认自动计算，最大80）  |
| `--dpi`                      | 150               | matplotlib savefig DPI |
...
│   ├── species_map.png
│   ├── species_deployment_comparison.png
│   ├── protection_deployment_comparison.png
│   └── fitness_history.png
├── scenario_2/
│   └── ...
...
> `--target-fitness` 和 `--target-benefit` **二选一必填**。
...
**文件**：`hexdynamic/time_risk_analyzer.py`

在单一地图基础上，分析 Day/Night × DRY/RAINY 四种时间组合的风险分布差异。
...
# 指定输出目录和 DPI
python time_risk_analyzer.py input.json -o ./time_risk_analysis --dpi 200 --grid_dpi 100
```
...
| :----------- | :--------------------- | :---------------- |
| `input`      | 必填                     | 输入 JSON 路径        |
| `-o`         | `./time_risk_analysis` | 输出目录              |
| `--dpi`      | 150                    | 保存图片的 DPI         |
| `--grid_dpi` | 自动计算                   | 每格像素宽度（默认自动计算）    |

#### 输出

| 文件                                   | 说明                          |
| :----------------------------------- | :-------------------------- |
| `{scenario}_norm_risk_heatmap.png`   | 各场景归一化风险热力图（4个）             |
| `{scenario}_raw_risk_heatmap.png`    | 各场景原始风险热力图（4个）              |
| `four_quadrant_comparison.png`       | 四象限原始风险对比图，四图共用一个颜色尺       |
| `summary_report.json`                | 分析摘要（各场景风险统计、时间因子、季节因子）     |

四种场景：Day+DRY、Day+RAINY、Night+DRY、Night+RAINY。
...
| `risk_comparison.png`        | 部署前后风险对比（上：原始风险，下：剩余风险），同色阶便于直接对比                  | YlOrRd |
...
| `species_deployment_comparison.png` | 上下布局：物种密度图 + 资源部署图                            |    —   |
| `protection_deployment_comparison.png` | 保护收益热力图 + 资源部署图                           |    —   |
| `fitness_history.png`        | Best fitness 随迭代次数的变化趋势                            |    —   |

有颜色条的图采用三列布局：**地图 | 颜色条 | 图例+指标**，三者互不遮挡。`risk_comparison.png` 采用上下排列布局，颜色条移至最左侧，summary 统计信息移至最右侧，颜色越深（红色）表示风险越高，对比上下可直观看出哪些高风险区域被有效覆盖。
...
**自动选择规则**：优先使用 OpenCV（如果已安装），否则尝试使用 FFmpeg，两者都不可用时提示错误。
...
| `--warm-start` | false | 启用热启动 |
| `--warm-start-groups` | 1 | 分组混合模式的并行组数 |
| `--no-cache` | false | 禁用缓存 |
...
```
figures/single_sensitivity/
├── sensitivity_results/             # 中间结果
│   ├── sensitivity_patrol.json
│   ├── sensitivity_camera.json
│   ├── sensitivity_{resource}_plot.png
│   └── .cache/
...
**收敛判断标准**：置信区间宽度 < 0.2 时认为该参数已收敛：`2 * conf_interval < 0.2`
...
> 以 `alpha_` 开头的参数自动注入 `coverage_params`，`total_` 开头的参数注入 `constraints`。
...
| 对比维度 | 7.2.1 资源总量 | 7.2.2 协同系数 | 7.2.3 覆盖度层面 |
| :--- | :--- | :--- | :--- |
...
2. **基础配置 JSON** 中的 `robustness_weights` 字段：
   ...
   {
     "robustness_weights": {
       "patrol": 0.4, "camp": 0.25, "drone": 0.2, "camera": 0.15
     },
     "grids": [...], "constraints": {...}
   }
   ```
...
> 若提供的权重之和不为 1.0，系统会自动归一化（每个权重除以总和）。
...
**`robustness_analysis.png`**：3 行 × 2 列共 6 个子图
...
**文件**：`analyze_resource_contribution.py`

通过假设场景分析各资源的边际收益，辅助排查资源利用率为 0% 的问题。

#### 用法

```bash
# 基本用法
python analyze_resource_contribution.py --input input.json --output output.json

# 指定输出目录
python analyze_resource_contribution.py --input input.json --output output.json --out-dir ./contribution_analysis

# 向量化模式
python analyze_resource_contribution.py --input input.json --output output.json --vectorized
```
...
| 参数             | 默认值                          | 说明                  |
| :------------- | :--------------------------- | :------------------ |
| `--input, -i`  | 必填                           | 输入 JSON 路径          |
| `--output, -o` | 必填                           | 输出 JSON 路径          |
| `--out-dir`    | `./contribution_analysis`    | 分析结果输出目录            |
| `--vectorized` | false                        | 使用向量化覆盖模型           |
...
### 2026-05-09：文档结构重组（v3.0）

- **文档版本升级至 3.0**：全面重组文档结构，按功能模块组织
- **新章节组织**：输入数据准备 → 输入 JSON 格式规范 → 风险模型 → 优化与部署 → 风险分析与可视化 → 分析工具
- **风险模型章节整合**：将综合风险公式、人为/环境/密度风险、时间因子、权重配置、部署约束体系统一放在第四章
- **时间因子计算模式详解**：新增 DISCRETE/CONTINUOUS 两种模式的详细说明和对比
- **可视化章节整合**：将风险分析、批量风险分析、时序风险对比、可视化输出、图片转视频统一放在第六章
- **分析工具章节整合**：将敏感性分析、Sobol 分析、蒙特卡洛鲁棒性、资源贡献分析统一放在第七章
- **输入 JSON 格式规范统一**：将分散在多处的参数说明合并到第三章，消除重复
...
- **新增根目录工具说明**：`marker_to_pipeline.py`、`analyze_resource_contribution.py`、`analyze_sobol_results.py`
- **新增 riskIndex 子系统工具说明**：`risk_model_wrapper.py`、`generate_hex_map.py` 等
- **补充 DSSA 优化器完整参数表**：包括停滞检测、适应度缓存、并行评估、资源迁移和全局重排操作
- **补充 HexGridModel 内存控制说明**：距离矩阵惰性加载、max_precompute_bytes 阈值、稀疏距离矩阵回退

### 2026-05-07：敏感性分析优化——热启动、缓存、分组混合策略、目录结构统一

- **sensitivity_analysis.py 新增参数**：`--workers`、`--two-step`、`--fine-step-ratio`、`--warm-start`、`--warm-start-groups`、`--no-cache`
- **sensitivity_batch.py 目录结构统一**：所有输出文件统一存放在 `--report-dir` 目录下
- **sensitivity_batch.py 启动前自动清理**：自动清理缓存目录和临时文件
- **dssa_optimizer.py 热启动支持**：新增 `warm_start_solution` 参数
- **protection_pipeline.py 热启动支持**：新增 `--warm-start` CLI 参数
- **Bug 修复**：`dssa_optimizer._perturb_solution` 中 rangers 扰动的 `dst_candidates` 误用

### 2026-05-02：新增多资源并行敏感性分析与 bug 修复

- **新增 `sensitivity_batch.py`**：并行运行多种资源的单因素敏感性分析
- **修复 `sensitivity_analysis.py` 的 `--resource all` bug**
- **文档更新**：重构敏感性分析章节为三个子章节

### 2026-05-02：新增协同系数与覆盖度层面 Sobol 敏感性分析

- **新增 `sobol_synergy_analysis.py`**：协同系数 Sobol 分析
- **新增 `sobol_coverage_analysis.py`**：覆盖度层面 Sobol 分析
- **文档更新**：重构 Sobol 分析章节为三个子章节

### 2026-05-02：权重参数统一配置

- **权重参数 JSON 化**：所有权重参数统一在输入 JSON 的 `risk_model_config` 中定义
- **新增 `temporal_weights` 分组**：昼夜因子和季节因子可通过 JSON 配置
- **修正人为风险默认权重**：与代码中 `HumanRiskWeights` 默认值一致
- **`generate_map.py` 新增 CLI 参数**：所有权重参数
- **Marker 导出面板新增权重输入**

### 2026-04-30：图片转视频工具与优化功能增强

- **新增 `images_to_video.py`**：支持 OpenCV 和 FFmpeg 两种后端
- **风险优先部署策略**：DSSA 配置新增 `use_risk_priority` 选项
- **迭代可视化优化**：异步绘制每轮迭代最优方案
- **网格编号配置**：输出 JSON 新增 `visualization_config.show_grid_ids`

### 2026-04-30：围栏边缘部署增强与可视化优化

- **围栏存储格式变更**：从 `(grid_id, None)` 改为 `(grid_id, direction)`
- **围栏部署规则优化**：支持按方向精确控制围栏部署
